...
try {
  ...
  const before = run("psql", [dbUrl, "-Atqc", "select count(*)||'|'||coalesce(string_agg(migration_name,',' order by migration_name),'') from _prisma_migrations"])
  ...
  const after = run("psql", [dbUrl, "-Atqc", "select count(*)||'|'||coalesce(string_agg(migration_name,',' order by migration_name),'') from _prisma_migrations"])
  if (before.trim() !== after.trim()) throw new Error("BLOCKED: second deploy changed migration metadata")
  const expectedNames = headFiles.map((file) => file.path.split("/")[2]).sort().join(",")
  if (after.trim().split("|")[1] !== expectedNames) throw new Error("BLOCKED: migration metadata names do not match committed history")
  ...
  console.log(`migration_ci=PASS base_history=${baseFiles.length} head_additions=${additions.length} metadata=${after.trim()} checksums=exact schema_diff=empty second_deploy=no-op`)
