...
describe("unit investor tone wiring", () => {
    it("uses the shared helper without local palette duplicates", () => {
        expect(pageSource).toContain('import { getInvestorTone } from "@/lib/investor-tone"')
    ...
    it("preserves both unit investor-tone call sites in the page", () => {
        expect(pageSource.match(/getInvestorTone\(unit\.investor\.name \|\| unit\.investorId\)/g)).toHaveLength(2)
    })
