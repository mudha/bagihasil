...
describe("PostgreSQL baseline repository contract", () => {
  it("declares PostgreSQL and preserves the canonical baseline", () => {
    ...
    expect(folders).toContain(manifest.baselineMigration)
    expect(folders.filter((name) => name !== manifest.baselineMigration).every((name) => /^\d{14}_[a-z0-9][a-z0-9_-]*$/.test(name))).toBe(true)
